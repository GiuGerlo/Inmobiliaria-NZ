                ...
                <div class="col-8 p-3">
                    <table class="table table-striped table-hover table-bordered" id="tabla-duenos">
                        ...
                        <tbody class="text-center">
                            <!-- CONSULTA A LA DB -->
                            <?php
                            $stmt = $conexion->prepare("SELECT dueno.*, ciudad.Nombre_Ciudad FROM dueno INNER JOIN ciudad ON dueno.CodP = ciudad.CodP ORDER BY ID_Dueno ASC");
                            $stmt->execute();
                            $duenos = $stmt->get_result();
