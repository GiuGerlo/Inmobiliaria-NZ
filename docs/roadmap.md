...
- **2026-06-19** — **Fusión NZ Fase 2 (dominio ventas) DONE**. 3 tablas inglés/snake_case (`property_types`/`sale_properties`/`property_images`) + modelos + factories; API REST con **lectura pública** (SSG) + **CRUD admin** (`auth:sanctum`), filtros spatie/query-builder, imágenes multi-upload WebP (reusa pipeline de `PropertyPhotoController`) + reorder; comando idempotente `ventas:import` (conexión `nzestudio`). Scramble autodocumenta los 8 endpoints. **Pest 151 verdes**; smoke real con el dump (7 tipos, 51 propiedades, 68 imágenes, 0 faltantes), API pública + imágenes sirviendo por HTTP 200. `/security-review` sin hallazgos. Próximo: Fase 3 (auth + roles `superadmin`/`inmobiliaria`).
