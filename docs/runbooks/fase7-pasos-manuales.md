...
Los **recordatorios masivos** (`SendBulkReminder`) van por cola `database` — un lote largo no puede
correr en el proceso web de shared hosting (lo mata). Este cron es un worker CLI efímero que drena la
cola. **Requerido para que los recordatorios masivos se envíen en el server.** Los recibos/rendiciones
...
> tras un deploy. `SendBulkReminder` es idempotente (solo procesa filas `queued`) + `tries=3` → si una
> corrida se corta a mitad del lote, la siguiente re-toma lo que falta.

---

## Bloque 8 — Datos de prueba en dev

- [ ] Sembrar datos demo por SSH (sin PII): `SalesDemoSeeder` (ventas) + `RentalDemoSeeder`
      (alquileres) + `RoleSeeder`. Ver receta en "Primer deploy a dev". Alternativa: importar un
      dump controlado a `nz_dev` (phpMyAdmin de Hostinger o por SSH) si querés más volumen.
...
2. Push a `dev` (o "Run workflow" manual de **deploy-api** primero). Esperar que termine OK:
   ...
   > El workflow corre `migrate --force` pero **NO seedea**. Tras el primer deploy, por SSH (con
   > `PHP=/opt/alt/php84/usr/bin/php`):
   > - `$PHP artisan db:seed --class=SalesDemoSeeder --force` → catálogo de ventas (necesario para que
   >   el público buildee; **sin factory**, corre OK en el server).
   > - `$PHP artisan db:seed --class=RentalDemoSeeder --force` → demo de alquileres (ciudades, dueños,
   >   inquilinos, propiedades, contratos, recibos). **Sin factory/Faker**, corre OK en el server. Idempotente.
   > - Crear el superadmin (los usuarios demo son local-only): `$PHP artisan tinker` y
   >   `User::firstOrNew(['Email_User'=>'<SUPERADMIN_EMAIL>'])` con `password = Hash::make('<pass>')`.
   > - `$PHP artisan db:seed --class=RoleSeeder --force` → crea roles + promueve `SUPERADMIN_EMAIL`.
   >
   > ⚠️ **`DemoSeeder` (demo de alquileres con datos random) NO corre en el server**: usa factories →
   > Faker, que es dep de **dev** y `composer --no-dev` no la instala. Por eso existe `RentalDemoSeeder`
   > (estático, sin Faker) para sembrar alquileres en dev/prod-demo. `DemoSeeder` queda solo para local.
